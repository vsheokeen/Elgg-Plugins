...
if (!empty($instanceid)) {
	...
	$Grade_id = $DB->get_record('grade_items',array('iteminstance'=> $lti->id , 'itemtype' => 'mod'));
	$Grade_items = $DB->get_record('grade_grades',array('itemid'=> $Grade_id->id , 'userid'=> $USER->id));
	//echo '<pre>';print_r($Grade_id);die;
	if(!empty($Grade_items))
	{
		$DB->set_field('grade_grades', 'finalgrade', $_REQUEST['points'], array('itemid'=> $Grade_id->id , 'userid'=> $USER->id));		
	}
	else
	{
		$grading = new stdClass();
		$grading->itemid = $Grade_id->id;
		$grading->userid = $USER->id;
		$grading->finalgrade = $_REQUEST['points'];
		$grading->timecreated = time();
		$grading->timemodified = time();
		$DB->insert_record('grade_grades', $grading, true);		
	}
