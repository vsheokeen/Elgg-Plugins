...
define("GROUPPOINTS_PER_HOUR", 10); 
...
function groups_init() {
	...
	elgg_register_action("groups/manage", "$action_base/manage.php"); /* custom */
	elgg_register_action("groups/groupbalance", "$action_base/groupbalance.php"); /* custom */
...
function groups_setup_sidebar_menus() {
	...
	/* custom */
	if(elgg_get_context() == 'group_profile' && $page_owner->canEdit() && elgg_is_logged_in())
	{
	elgg_register_menu_item('page', array(
			'name' => 'grouppoints_balance',
			'text' => elgg_echo('Manage Group Points'),
			'href' => 'groups/manage/'.$page_owner->getGUID(),
	));

	}
	
	if(elgg_get_context() == 'group_profile' && elgg_is_logged_in() && $page_owner->isMember(elgg_get_logged_in_user_entity()))
	{
	elgg_register_menu_item('page', array(
			'name' => 'group_balance',
			'text' => elgg_echo('Group Points Balance'),
			'href' => 'groups/groupbalance/'.$page_owner->getGUID(),
	));
	}
