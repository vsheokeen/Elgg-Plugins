...
function groups_handle_manage_page($page, $guid = 0)
{
	elgg_gatekeeper();

	$title = 'Manage Group Points';
	...
	if ($page == 'manage') {
		...
		$content = elgg_view("groups/manage", array('container_guid' => $guid));
...
function groups_handle_groupbalance_page($page, $guid = 0)
{
	elgg_gatekeeper();

	$title = elgg_echo('grouppoints_balance:title');
	
	if ($page == 'groupbalance') {
		elgg_set_page_owner_guid(elgg_get_logged_in_user_guid());
		
		elgg_push_breadcrumb($title);
	
		$content = elgg_view("groups/groupbalance", array('container_guid' => $guid));
